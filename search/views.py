...
def searchView(request, page):
    if request.method == 'GET':
        ...
        if kword:
            # Q是SQL语句里的or语法
            song_info = Song.objects.values('song_id', 'song_name', 'song_singer', 'song_time').filter(Q(song_name__icontains=kword) | Q(song_singer__icontains=kword) | Q(song_album__contains=kword) | Q(song_type__icontains=kword) | Q(song_languages__icontains=kword)).order_by('-song_release').all()
        else:
